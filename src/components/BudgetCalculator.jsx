...
export default function BudgetCalculator() {
  ...
  const [perDayBase, setPerDayBase] = useState(15000) // base per-day budget (PKR)
  ...
  const totalAddons = useMemo(() => Math.max(0, days) * (Math.max(0, addons.activities) + Math.max(0, addons.buffer)), [days, addons])
  ...
  const normalized = sumAlloc === 0 ? { food: 0, stay: 0, transport: 0, misc: 0 } : {
    ...
    <div className="tile">
      ...
      <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-5 gap-3 text-sm">
        <div className="tile p-3">
          ...
          <div className="text-xs text-slate-500">Base: {formatCurrency(totalBase)} • Addons: {formatCurrency(totalAddons)}</div>
        </div>
        <div className="tile p-3"><span className="font-semibold">Food</span><div>{formatCurrency(breakdown.food)}</div></div>
        <div className="tile p-3"><span className="font-semibold">Stay</span><div>{formatCurrency(breakdown.stay)}</div></div>
        <div className="tile p-3"><span className="font-semibold">Transport</span><div>{formatCurrency(breakdown.transport)}</div></div>
        <div className="tile p-3"><span className="font-semibold">Misc</span><div>{formatCurrency(breakdown.misc)}</div></div>
      </div>
      ...
      <div className="mt-4 flex gap-2">
        <button className="btn-primary" onClick={apply}>Save budget</button>
